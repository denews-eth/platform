        ...
        <div class="col-12 col-xl-9">
          ...
          <div class="profile p-3" v-if="!verified && twitter">
            <div class="row justify-content-center">
              <p class="mt-2 mr-3" style="color: #dedede">
                Profilo non verificato!
              </p>
              <a
                class="author__follow"
                style="background: #9e550b"
                href="/verify"
                >Verifica</a
              >
            </div>
          ...
          <div class="tab-content" v-if="twitter">
              ...
              <div class="row row--grid">
                ...
                >
                  <ArticlePreview
                    :article="article"
                    :author_image="
                      search(article.author, users) != undefined
                        ? search(article.author, users).profile_image_url
                        : ''
                    "
                    v-on:article_saved="
                      editProfile(
            ...
            >
              <div class="row row--grid">
                <div
                  class="col-12 col-sm-6 col-lg-4"
                  v-for="article in articlesSaved"
                  :key="article.hash"
                >
                  <ArticlePreview
                    :article="article"
                ...
                <div class="col-12">
                  ...
                  >
                    <div class="row">
                        ...
                        <div class="mb-3" style="height: 200px">
                          <div class="sign__file" style="height: 100%">
                            <label
                              ...
                              class="pl-5 pr-4"
                              v-show="preview_image.length == 0"
                              style="height: 100%"
                              >Upload your profile image (400x400)
                        ...
                        <div class="mb-3" style="height: 200px">
                          <div class="sign__file" style="height: 100%">
                            <label
                              for="profile_banner"
                              class="p-5"
                              v-show="profile_banner.length == 0"
                              style="height: 100%"
                              >Upload your profile image (1500x500)
                              <i class="fa fa-upload mx-2"></i
                            ></label>
                            <input
                              data-name="#profile_banner"
                              v-show="profile_banner.length == 0"
                              @change="bannerImage()"
                              id="profile_banner"
                              name="profile_banner"
                      ...
                      <div class="col-12">
                        <button
                          class="sign__btn ml-1"
                          type="button"
                          ref="editBtn"
                          @click="updateProfile()"
                        >
                          Save <i class="fa fa-spinner ml-2" v-if="loading"></i>
                        </button>
                      </div>
                    </div>
                  </form>
          ...
          <div class="tab-content row flex-column" v-if="!twitter">
            <div class="mx-auto">
              ...
              <a
                class="author__follow"
                type="button"
                style="padding: 15px; margin: 0 auto; width: 200px"
                :href="apiURL + '/twitter/login'"
                >Accedi con Twitter</a
              >
            </div>
          </div>
          <!-- end content tabs -->
...
export default {
  ...
  methods: {
    ...
    async updateProfile() {
      ...
      if (this.preview_image.length > 0 && this.profile_banner.length > 0) {
        ...
        if (document.getElementById("profile_image_url").files[0] != null) {
          formData.append(
            "images",
            document.getElementById("profile_image_url").files[0],
            "profile_img"
          );
        }
        if (document.getElementById("profile_banner").files[0] != null) {
          formData.append(
            "images",
            document.getElementById("profile_banner").files[0],
            "profile_banner_img"
          );
        }
        let res = await axios.post("/users/edit", formData, {
          headers: {
    ...
    bannerImage() {
      setTimeout(() => {
        this.profile_banner = URL.createObjectURL(
          document.getElementById("profile_banner").files[0]
        );
      }, 500);
    },
      ...
      if (this.oauth_token !== "") {
        if (saved == true) {
          ...
          if (res.data.error == true) return;
            ...
            this.articlesSaved[
              this.articlesSaved.findIndex(({ hash }) => hash === article.hash)
            ] = undefined;
            let i2 = 0;
            let j2 = 0;
